...
def _direction_label(degrees: int) -> str:
...
def score_hour(site: LaunchSite, weather: WeatherHour, pilot: PilotProfile) -> FlyabilityHour:
    ...
    wind_label = _direction_label(weather.wind_direction_deg)

    # Wind direction vs safe sectors
    if wind_label not in site.safe_directions:
        blockers.append(Blocker(
            code="UNSAFE_DIRECTION", severity="hard",
            message=f"Wind {wind_label} is outside safe sectors ({', '.join(site.safe_directions)}).",
        ))
        safety_score -= 45
        flyability_score -= 35
    else:
    ...
    if weather.wind_kmh > pilot.max_wind_kmh:
        blockers.append(Blocker(
            code="WIND_LIMIT", severity="hard",
            message=f"Wind {weather.wind_kmh:.0f} km/h exceeds pilot limit {pilot.max_wind_kmh} km/h.",
        ))
        safety_score -= 45
        flyability_score -= 40
    elif weather.wind_kmh > pilot.max_wind_kmh * 0.8:
        blockers.append(Blocker(
            code="WIND_MARGIN", severity="warning",
            message=f"Wind {weather.wind_kmh:.0f} km/h is close to the pilot limit.",
        ))
        safety_score -= 18
        flyability_score -= 15
    else:
    ...
    if weather.gust_kmh > pilot.max_gust_kmh:
        blockers.append(Blocker(
            code="GUST_LIMIT", severity="hard",
            message=f"Gust {weather.gust_kmh:.0f} km/h exceeds pilot limit {pilot.max_gust_kmh} km/h.",
        ))
        safety_score -= 50
        flyability_score -= 45
    elif weather.gust_kmh - weather.wind_kmh > 12:
        blockers.append(Blocker(
            code="GUST_SPREAD", severity="warning",
            message="Gust spread >12 km/h suggests variable launch conditions.",
        ))
        safety_score -= 16
        flyability_score -= 12

    # Cloud base
    min_cloudbase = site.altitude_m + 300
    if weather.cloudbase_msl_m < min_cloudbase:
        blockers.append(Blocker(
            code="LOW_CLOUDBASE", severity="hard",
            message=f"Cloudbase {weather.cloudbase_msl_m} m is below safe margin {min_cloudbase} m MSL.",
        ))
        safety_score -= 40
        flyability_score -= 30
    else:
        reasons.append(f"Cloudbase {weather.cloudbase_msl_m} m provides adequate clearance.")

    # Storm risk (hard gate at ≥60 %)
    if weather.storm_risk >= 60:
        blockers.append(Blocker(
            code="STORM_RISK", severity="hard",
            message=f"Storm risk {weather.storm_risk}% is a hard safety blocker.",
        ))
        safety_score = min(safety_score, 20)
        flyability_score = min(flyability_score, 15)
    elif weather.storm_risk >= 30:
        blockers.append(Blocker(
            code="STORM_WATCH", severity="warning",
            message=f"Storm risk {weather.storm_risk}% requires active monitoring.",
        ))
        safety_score -= 22
    ...
    if weather.rain_mm > 0.5:
        blockers.append(Blocker(
            code="PRECIPITATION", severity="hard",
            message=f"Rain {weather.rain_mm:.1f} mm/h is incompatible with normal operations.",
        ))
        safety_score -= 50
        flyability_score -= 45
    ...
    confidence = 0.85 if status != DecisionStatus.UNKNOWN else 0.3
    explanation = reasons + [b.message for b in blockers]
